...
import {
  uiUxCaseStudies as caseStudies,
  uiUxDesignTools as designTools,
  uiUxSteps as steps,
  uiUxWayOfWorkingCards,
  uiUxStats,
} from '@/lib/services/uiUxDesignConfig';

// Data and animations are imported from their respective config/utility files
  ...
  return (
    <main className="min-h-screen bg-black text-white selection:bg-red-500/30 font-sans overflow-x-hidden scroll-smooth">
      ...
      >
        ...
        <h1 className="text-6xl md:text-7xl font-bold tracking-tighter mb-4">UI/UX Design</h1>
        <p className="text-gray-400 text-sm md:text-base mb-16 tracking-wide">Designing Experiences That Drive Business Success</p>
        ...
        <div className="max-w-6xl mx-auto p-12 md:p-20 rounded-[40px] bg-[#0a0a0a] border border-red-900/20 backdrop-blur-xl relative">
          ...
          <h2 className="text-3xl md:text-4xl font-bold mb-6 tracking-tight mt-4">Scalable Code, Reliable Solutions</h2>
          <p className="max-w-4xl mx-auto text-gray-400 leading-relaxed text-sm md:text-base mb-10">
            At HoocAI Solution, we don&apos;t just write code; we build digital foundations. We ensure security, speed, and cross-browser compatibility are baked into every line of code.
          </p>
      ...
      >
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          {uiUxWayOfWorkingCards.map((card, i) => (
            <motion.div 
              variants={fadeInUp}
            ...
            >
              <h3 className="text-lg font-bold mb-4 tracking-tight leading-tight">{card.t}</h3>
        ...
        <div className="relative">
          ...
          <div className="space-y-6">
            {steps.map((step, idx) => (
              <motion.div 
                whileHover={{ x: 10 }}
                key={idx} className="flex gap-8 items-center p-6 rounded-[24px] bg-[#0a1619] border border-cyan-900/30 group relative z-10 transition-all"
              >
                <div className="flex-shrink-0 w-12 h-12 rounded-full border-2 border-red-500 bg-[#0a1619] flex items-center justify-center font-bold text-lg transition-transform group-hover:scale-110">
                  {step.id}
                </div>
                <div className="flex-1 text-left">
                  <h4 className="font-bold mb-1 text-lg group-hover:text-red-300 transition-colors">{step.t}</h4>
      ...
      <motion.section id="portfolio" className="py-24 px-6 bg-black">
        <div className="max-w-6xl mx-auto text-center">
              ...
              <motion.div key={i} whileHover={{y: -10}} className="bg-[#0a0a0a] rounded-[40px] p-4 border border-white/5 group hover:border-red-500/30 transition-all">
                ...
                <div className="px-4 pb-6 flex justify-between items-start">
                  <div>
                    ...
                    <p className="text-gray-500 text-xs mt-2 line-clamp-2">{project.desc}</p>
                  </div>
          ...
          <div className="mt-16">
            <Link href="/portfolios" className="inline-flex bg-white text-black px-10 py-3 rounded-full text-xs font-bold items-center gap-2 hover:bg-red-600 hover:text-white transition-all transform hover:scale-105">
      ...
      <section className="py-20 flex flex-col items-center">
        ...
        <div className="flex flex-wrap justify-center gap-8 md:gap-12 px-6 max-w-5xl">
          {designTools.map((tool, idx) => (
            <motion.div key={idx} variants={bubbleAnimation} animate="animate" className="w-16 h-16 md:w-20 md:h-20 bg-[#111] rounded-2xl border border-white/5 flex items-center justify-center p-4 group">
      ...
      <motion.section className="py-24 px-6 max-w-7xl mx-auto border-t border-white/5">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-20 items-center">
          ...
          <div className="text-left">
             <span className="inline-block border border-red-900/50 bg-red-950/20 text-red-500 px-3 py-1 rounded-md text-[9px] font-bold uppercase mb-6">Success Rate</span>
             <h2 className="text-5xl font-bold tracking-tighter text-white">Delivering Measurable Success.</h2>
          </div>
        </div>
      </motion.section>
